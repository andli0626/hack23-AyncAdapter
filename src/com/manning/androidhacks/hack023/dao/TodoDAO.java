...
public class TodoDAO {
	...
	public List<Todo> getCleanTodos(ContentResolver contentResolver) {
		return getTodosWithSelection(contentResolver,TodoContentProvider.COLUMN_STATUS_FLAG + " = "+ StatusFlag.CLEAN);
	}
	...
	public int deleteTodo(ContentResolver contentResolver, Long id) {
		...
		switch (status) {
		case StatusFlag.ADD:
			ret = contentResolver.delete(TodoContentProvider.CONTENT_URI,TodoContentProvider.COLUMN_ID + "=" + id, null);
			break;
		case StatusFlag.MOD:
	...
	public int getTodoStatus(ContentResolver contentResolver, Long id) {
		Cursor c = contentResolver.query(TodoContentProvider.CONTENT_URI, null,TodoContentProvider.COLUMN_ID + "=" + id, null, null);
	...
	public int forcedDeleteTodo(ContentResolver contentResolver, Long id) {
		return contentResolver.delete(TodoContentProvider.CONTENT_URI,TodoContentProvider.COLUMN_SERVER_ID + "=" + id, null);
	}

	public Todo isTodoInDb(ContentResolver contentResolver, Long serverId) {
		Todo todo = null;

		Cursor cursor = contentResolver.query(TodoContentProvider.CONTENT_URI,null, TodoContentProvider.COLUMN_SERVER_ID + "=" + serverId,null, null);
	...
	public void modifyTodoFromServer(ContentResolver contentResolver, Todo list) {
		...
		contentResolver.update(TodoContentProvider.CONTENT_URI,cv,TodoContentProvider.COLUMN_SERVER_ID + "="+ list.getId(), null);

	}

	public List<Todo> getDirtyList(ContentResolver mContentResolver) {
		return getTodosWithSelection(mContentResolver,TodoContentProvider.COLUMN_STATUS_FLAG + " != "+ StatusFlag.CLEAN);
	}
